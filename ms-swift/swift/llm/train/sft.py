...
class SwiftSft(SwiftPipeline, TunerMixin):
    ...
    def _prepare_gradient_checkpointing(self):
        ...
        # 确保 Gradient Checkpointing 仅应用一次
        if args.gradient_checkpointing and not getattr(self.model, "_gradient_checkpointing_applied", False):
            self.model.supports_gradient_checkpointing = True
            dynamic_gradient_checkpointing(self.model)
            self.model.enable_input_require_grads()
            self.model._gradient_checkpointing_applied = True  # 标记已应用
